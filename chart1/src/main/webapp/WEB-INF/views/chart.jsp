...
$(document).ready(function(){
	...
	var lineChartData = { //그래프를 그릴수 있도록 가공해둔 변수(차트를 만들때 넣을 변수)
	...
	}; //lineChartData (객체형 변수)
	
	function createChart()  {
		var ctx = document.getElementById("canvas").getContext("2d"); //도화지 객체
		LineChartDemo = Chart.Line(ctx,{ //Chart.Line()메서드는 Chartjs에서 제공한 선그래프그리기 메서드
			data : lineChartData, //chartLabels,chartData1, chartData2
			options : {
				scales : { //눈금표시
					yAxes : [ //y축으로 눈금표시
						{
	...
	$.ajax({
        ...
        success : function(result){ //result는 서버로부터 받은 값(json object=map)
        	$.each(result.datas,function(index, obj){ //자바의 enhanced for문을 생각 할것
        		...
        		chartLabels.push(obj.month); //배열에 값을 넣어줌 //5월
                chartData1.push(obj.pc);	//40
                chartData2.push(obj.monitor);	//110
        	});	//each문을 통해 빈 객체 chartLabels,chartData1, chartData2에 데이터 채워줌
